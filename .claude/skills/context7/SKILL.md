...
2. Add or update the entry with:
   ...
   - Version currently in use (from CMakeLists.txt, idf_component.yml, or component registry)
   - Date last verified
...
At the **start of each session** and whenever you read a dependency file (CMakeLists.txt, idf_component.yml, etc.):

1. Read the registry at `.claude/context7-registry.md`
2. Read the project's dependency file(s) (e.g., CMakeLists.txt for FetchContent deps, idf_component.yml for ESP component registry deps)
